...
const ALLOWED_QUERY_BLOCKS = CONFIG.allowedQueryBlocks;
const TEMPLATE = CONFIG.template;
const TEMPLATEQUERY = CONFIG.templateQuery;
...
function SliderEdit(props) {
	...
	const {innerContent, innerBlocks, getBlockOrder } = useSelect(
		(select) => {
	...
	const { removeBlocks,removeBlock  } = useDispatch( blockEditorStore  );
	
	// check for duplicates + set blockId
	handleBlockId(props);
	
	// handle add / remove changes
	const [isQuerySlider, setIsQuerySlider] = useState(false);
	const [childrenCount, setChildrenCount] = useState(0);
	const [isRemovingInnerBlocks, setIsRemovingInnerBlocks] = useState(false);
	
	useEffect(() => {
		// detect if a query block got added
		const allowedQueryBlocks = innerBlocks.filter(block => ALLOWED_QUERY_BLOCKS.includes(block.name));
		if (allowedQueryBlocks.length > 0) {
			setIsQuerySlider(true);
		}
		
		// set childrenCount
		setChildrenCount(innerBlocks.length);
	}, [innerBlocks]);

	// if isQuery changed handle inner blocks
	useEffect(() => {
		if(isQuerySlider){
			if(confirm("Adding a query type block will delete all current slides! Are you sure?")){
				// remove other blocks
				removeBlocks(
					innerBlocks.filter(block => !ALLOWED_QUERY_BLOCKS.includes(block.name)).map(block => block.clientId)
				);
			}else{
				// remove only query blocks
				removeBlocks(
					innerBlocks.filter(block => ALLOWED_QUERY_BLOCKS.includes(block.name)).map(block => block.clientId)
				);
				setIsQuerySlider(false);
			}
		}
	}, [isQuerySlider]);
	
	useEffect(() => {
		setSliderAttributes(props, { isQuerySlider, childrenCount });
	}, [props, isQuerySlider, childrenCount]);
	...
	if (editMode === true) {
		...
		return (
			<>
				...
				<TagName {...blockProps}>
					{ isQuerySlider ? <InnerBlocks {...innerBlocksProps} template={TEMPLATEQUERY} allowedBlocks={ALLOWED_QUERY_BLOCKS}/>
						: <ul>
