			...
			<PanelBody title='Navigation'>
				...
				<Flex justify={'flex-start'}>
					<FlexItem>
						<ColorIndicator
							className={'clickable'}
							colorValue={svSliderAttributes['--swiffy-slider-arrows-colors'][currentResponsiveTab]}
							onClick={()=>props.setAttributes({_svSliderArrowsColorsPopover: true})}
						/>
						{ attributes._svSliderArrowsColorsPopover === true &&
							<Popover position='left' onClose={()=>props.setAttributes({_svSliderArrowsColorsPopover: false})}>
								<ColorPalette
									colors={ themeColors }
									value={ svSliderAttributes['--swiffy-slider-arrows-colors'][currentResponsiveTab] }
									onChange={ (value) => onChange(value, '--swiffy-slider-arrows-colors') }
								/>
							</Popover>
						}
					</FlexItem>
					<FlexItem>
						<div>{__('Color', 'sv_slider')}</div>
					</FlexItem>
				</Flex>
			</PanelBody>
			{/*
